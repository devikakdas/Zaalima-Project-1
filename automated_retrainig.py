...
class AutomatedRetrainingPipeline:
    ...
    def prepare_training_data(self, df, lookback_days=90, test_size=0.2):
        """
        Prepare data for retraining with stratified split
        Ensures both classes present in test set
        """
        ...
        logger.info(f"\nData preparation:")
        logger.info(f"  Total samples: {len(df_recent):,}")

        # Separate failures and normal operations
        failures = df_recent[df_recent['failure_within_24h'] == 1]
        normals = df_recent[df_recent['failure_within_24h'] == 0]

        logger.info(f"  Failures: {len(failures):,} ({len(failures)/len(df_recent):.2%})")
        logger.info(f"  Normal: {len(normals):,} ({len(normals)/len(df_recent):.2%})")

        # Check if we have enough failures
        if len(failures) < 100:
            raise ValueError(f"Insufficient failures for training: {len(failures)} (need at least 100)")

        # Stratified temporal split (take last 20% of each class for testing)
        n_test_failures = max(int(len(failures) * test_size), 50)  # At least 50 failures
        n_test_normals = int(len(normals) * test_size)

        # Get latest samples from each class for testing
        test_failures = failures.tail(n_test_failures)
        test_normals = normals.tail(n_test_normals)

        # Get remaining for training
        ...
        logger.info(f"✓ Test set:  {len(X_test):,} samples ({y_test.mean():.2%} failure rate)")

        # Verify both classes present
        train_classes = len(y_train.unique())
        test_classes = len(y_test.unique())

        if train_classes < 2:
            raise ValueError(f"Train set has only one class! Classes: {y_train.unique()}")
        if test_classes < 2:
            raise ValueError(f"Test set has only one class! Classes: {y_test.unique()}")

        logger.info(f"✓ Both classes present in train and test sets")
    ...
    def evaluate_models(self, current_model, new_model, X_test, y_test):
        ...
        if len(y_test.unique()) < 2:
            logger.error(f"❌ Test set has only one class: {y_test.unique()}")
            raise ValueError("Cannot evaluate: test set needs both failure and normal samples")
        ...
        try:
            ...
            logger.error(f"❌ Failed to evaluate current model: {str(e)}")
            raise
        ...
        try:
            ...
            logger.error(f"❌ Failed to evaluate new model: {str(e)}")
            raise
